...
import com.dskja.betterstreamflix.extractors.VixcloudExtractor
...
import com.dskja.betterstreamflix.utils.InertiaUtils
...
class StreamingCommunityProvider(private val _language: String? = null) : Provider {
    ...
    override suspend fun getServers(id: String, videoType: Video.Type): List<Video.Server> {
        Log.i("BetterStreamflix", "[PROV] -> StreamingCommunity: getServers for $id")
        ...
        Log.i("BetterStreamflix", "[PROV] -> Found Vixcloud src: $src")
        return listOf(Video.Server(id = id, name = "Vixcloud", src = src))
    }

    override suspend fun getVideo(server: Video.Server): Video {
        Log.i("BetterStreamflix", "[SERVER] -> StreamingCommunity: Using ${server.name} (Src: ${server.src})")
